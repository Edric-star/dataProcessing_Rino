...
def process_files(input_point_cloud_folder, input_label_folder, input_semantic_label_folder, output_point_cloud_folder, output_semantic_label_folder, labels_to_remove):
    ...
    point_cloud_files = set(os.listdir(input_point_cloud_folder))
    label_files = set(os.listdir(input_label_folder))
    semantic_label_files = set(os.listdir(input_semantic_label_folder))
    ...
    print(f"共找到 {len(common_basenames)} 个共有文件")

    for basename in common_basenames:
        point_cloud_file_path = os.path.join(input_point_cloud_folder, basename + '.bin')
        label_file_path = os.path.join(input_label_folder, basename + '.bin')
        semantic_label_file_path = os.path.join(input_semantic_label_folder, basename + '.label')

        output_point_cloud_file_path = os.path.join(output_point_cloud_folder, basename + '.bin')
        output_semantic_label_file_path = os.path.join(output_semantic_label_folder, basename + '.label')
...
# 文件夹路径
input_point_cloud_folder = "/home/edric/dataProcess/107_11_pcd_float32bin"
input_label_folder = "/media/edric/c0aeda71-48a7-463b-9fb2-a7d90e7e1233/rino_107/others_info/clip_rino_107_11/pnt_instruction"
input_semantic_label_folder = "/home/edric/dataProcess/107_11_uint32label"
output_point_cloud_folder = "/home/edric/dataProcess/107_11_pcd_float32bin_withoutCm"
output_semantic_label_folder = "/home/edric/dataProcess/107_11_uint32label_withoutCm"
