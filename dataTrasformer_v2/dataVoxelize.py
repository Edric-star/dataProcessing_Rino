...
def main():
    base_folder = '/home/edric/PaSCo_Edric/gpfsdswork/dataset/SemanticKITTI/dataset/sequences'
    ...
    # 遍历所有子文件夹
    for sequence in os.listdir(base_folder):
        sequence_path = os.path.join(base_folder, sequence)
        if os.path.isdir(sequence_path):
            bin_folder = os.path.join(sequence_path, 'velodyne')
            label_folder = os.path.join(sequence_path, 'labels')
            output_dir = os.path.join(sequence_path, 'voxels')

            if os.path.exists(bin_folder) and os.path.exists(label_folder):
